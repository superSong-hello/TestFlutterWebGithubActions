///
//  Generated code. Do not modify.
//  source: label/v1/label.proto
//
// @dart = 2.12
// ignore_for_file: annotate_overrides,camel_case_types,constant_identifier_names,deprecated_member_use_from_same_package,directives_ordering,library_prefixes,non_constant_identifier_names,prefer_final_fields,return_of_invalid_type,unnecessary_const,unnecessary_import,unnecessary_this,unused_import,unused_shown_name

import 'dart:core' as $core;
import 'dart:convert' as $convert;
import 'dart:typed_data' as $typed_data;

@$core.Deprecated('Use getPrintResultRequestDescriptor instead')
const GetPrintResultRequest$json = {
  '1': 'GetPrintResultRequest',
  '2': [
    {
      '1': 'store_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'storeCode',
      '17': true
    },
    {
      '1': 'prod_num',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'prodNum',
      '17': true
    },
    {
      '1': 'data_list',
      '3': 3,
      '4': 3,
      '5': 11,
      '6': '.label.v1.GetPrintResultRequest.Data',
      '10': 'dataList'
    },
  ],
  '3': [GetPrintResultRequest_Data$json],
  '8': [
    {'1': '_store_code'},
    {'1': '_prod_num'},
  ],
};

@$core.Deprecated('Use getPrintResultRequestDescriptor instead')
const GetPrintResultRequest_Data$json = {
  '1': 'Data',
  '2': [
    {
      '1': 'product_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'productCode',
      '17': true
    },
    {
      '1': 'print_num',
      '3': 2,
      '4': 1,
      '5': 13,
      '9': 1,
      '10': 'printNum',
      '17': true
    },
    {
      '1': 'print_size_id',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'printSizeId',
      '17': true
    },
    {
      '1': 'title_id',
      '3': 4,
      '4': 1,
      '5': 9,
      '9': 3,
      '10': 'titleId',
      '17': true
    },
    {
      '1': 'pattern_id',
      '3': 5,
      '4': 1,
      '5': 9,
      '9': 4,
      '10': 'patternId',
      '17': true
    },
  ],
  '8': [
    {'1': '_product_code'},
    {'1': '_print_num'},
    {'1': '_print_size_id'},
    {'1': '_title_id'},
    {'1': '_pattern_id'},
  ],
};

/// Descriptor for `GetPrintResultRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getPrintResultRequestDescriptor = $convert.base64Decode(
    '');
@$core.Deprecated('Use getPrintResultResponseDescriptor instead')
const GetPrintResultResponse$json = {
  '1': 'GetPrintResultResponse',
  '2': [
    {'1': 'code', '3': 1, '4': 1, '5': 9, '9': 0, '10': 'code', '17': true},
    {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
    {
      '1': 'table0',
      '3': 3,
      '4': 1,
      '5': 11,
      '6': '.label.v1.GetPrintResultResponse.Table0',
      '10': 'table0'
    },
  ],
  '3': [GetPrintResultResponse_Table0$json],
  '8': [
    {'1': '_code'},
    {'1': '_message'},
  ],
};

@$core.Deprecated('Use getPrintResultResponseDescriptor instead')
const GetPrintResultResponse_Table0$json = {
  '1': 'Table0',
  '2': [
    {'1': 'count', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'count', '17': true},
    {
      '1': 'rows',
      '3': 2,
      '4': 3,
      '5': 11,
      '6': '.label.v1.GetPrintResultResponse.Table0.Row',
      '10': 'rows'
    },
  ],
  '3': [GetPrintResultResponse_Table0_Row$json],
  '8': [
    {'1': '_count'},
  ],
};

@$core.Deprecated('Use getPrintResultResponseDescriptor instead')
const GetPrintResultResponse_Table0_Row$json = {
  '1': 'Row',
  '2': [
    {'1': 'row_no', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'rowNo', '17': true},
    {
      '1': 'print_result_id',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'printResultId',
      '17': true
    },
    {
      '1': 'print_result',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'printResult',
      '17': true
    },
  ],
  '8': [
    {'1': '_row_no'},
    {'1': '_print_result_id'},
    {'1': '_print_result'},
  ],
};

/// Descriptor for `GetPrintResultResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getPrintResultResponseDescriptor =
    $convert.base64Decode(
        '');
@$core.Deprecated('Use labelGetFormatsRequestDescriptor instead')
const LabelGetFormatsRequest$json = {
  '1': 'LabelGetFormatsRequest',
  '2': [
    {
      '1': 'store_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'storeCode',
      '17': true
    },
  ],
  '8': [
    {'1': '_store_code'},
  ],
};

/// Descriptor for `LabelGetFormatsRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List labelGetFormatsRequestDescriptor =
    $convert.base64Decode(
        'ChZMYWJlbEdldEZvcm1hdHNSZXF1ZXN0EiIKCnN0b3JlX2NvZGUYASABKAlIAFIJc3RvcmVDb2RliAEBQg0KC19zdG9yZV9jb2Rl');
@$core.Deprecated('Use labelGetFormatsResponseDescriptor instead')
const LabelGetFormatsResponse$json = {
  '1': 'LabelGetFormatsResponse',
  '2': [
    {'1': 'code', '3': 1, '4': 1, '5': 9, '9': 0, '10': 'code', '17': true},
    {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
    {
      '1': 'max_num',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'maxNum',
      '17': true
    },
    {
      '1': 'max_print_num',
      '3': 4,
      '4': 1,
      '5': 9,
      '9': 3,
      '10': 'maxPrintNum',
      '17': true
    },
    {
      '1': 'table0',
      '3': 5,
      '4': 1,
      '5': 11,
      '6': '.label.v1.LabelGetFormatsResponse.Table0',
      '10': 'table0'
    },
  ],
  '3': [LabelGetFormatsResponse_Table0$json],
  '8': [
    {'1': '_code'},
    {'1': '_message'},
    {'1': '_max_num'},
    {'1': '_max_print_num'},
  ],
};

@$core.Deprecated('Use labelGetFormatsResponseDescriptor instead')
const LabelGetFormatsResponse_Table0$json = {
  '1': 'Table0',
  '2': [
    {'1': 'count', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'count', '17': true},
    {
      '1': 'rows',
      '3': 2,
      '4': 3,
      '5': 11,
      '6': '.label.v1.LabelGetFormatsResponse.Table0.Row',
      '10': 'rows'
    },
  ],
  '3': [LabelGetFormatsResponse_Table0_Row$json],
  '8': [
    {'1': '_count'},
  ],
};

@$core.Deprecated('Use labelGetFormatsResponseDescriptor instead')
const LabelGetFormatsResponse_Table0_Row$json = {
  '1': 'Row',
  '2': [
    {'1': 'row_no', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'rowNo', '17': true},
    {'1': 'id', '3': 2, '4': 1, '5': 5, '9': 1, '10': 'id', '17': true},
    {
      '1': 'variety_cd',
      '3': 3,
      '4': 1,
      '5': 5,
      '9': 2,
      '10': 'varietyCd',
      '17': true
    },
    {
      '1': 'title_id',
      '3': 4,
      '4': 1,
      '5': 5,
      '9': 3,
      '10': 'titleId',
      '17': true
    },
    {
      '1': 'title_name',
      '3': 5,
      '4': 1,
      '5': 9,
      '9': 4,
      '10': 'titleName',
      '17': true
    },
    {
      '1': 'variety_name',
      '3': 6,
      '4': 1,
      '5': 9,
      '9': 5,
      '10': 'varietyName',
      '17': true
    },
    {
      '1': 'pattern_id',
      '3': 7,
      '4': 1,
      '5': 5,
      '9': 6,
      '10': 'patternId',
      '17': true
    },
    {
      '1': 'pattern_name',
      '3': 8,
      '4': 1,
      '5': 9,
      '9': 7,
      '10': 'patternName',
      '17': true
    },
  ],
  '8': [
    {'1': '_row_no'},
    {'1': '_id'},
    {'1': '_variety_cd'},
    {'1': '_title_id'},
    {'1': '_title_name'},
    {'1': '_variety_name'},
    {'1': '_pattern_id'},
    {'1': '_pattern_name'},
  ],
};

/// Descriptor for `LabelGetFormatsResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List labelGetFormatsResponseDescriptor =
    $convert.base64Decode(
        '');
@$core.Deprecated('Use getProdInfoRequestDescriptor instead')
const GetProdInfoRequest$json = {
  '1': 'GetProdInfoRequest',
  '2': [
    {
      '1': 'product_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'productCode',
      '17': true
    },
    {
      '1': 'store_code',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'storeCode',
      '17': true
    },
  ],
  '8': [
    {'1': '_product_code'},
    {'1': '_store_code'},
  ],
};

/// Descriptor for `GetProdInfoRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getProdInfoRequestDescriptor = $convert.base64Decode(
    'ChJHZXRQcm9kSW5mb1JlcXVlc3QSJgoMcHJvZHVjdF9jb2RlGAEgASgJSABSC3Byb2R1Y3RDb2RliAEBEiIKCnN0b3JlX2NvZGUYAiABKAlIAVIJc3RvcmVDb2RliAEBQg8KDV9wcm9kdWN0X2NvZGVCDQoLX3N0b3JlX2NvZGU=');
@$core.Deprecated('Use getProdInfoResponseDescriptor instead')
const GetProdInfoResponse$json = {
  '1': 'GetProdInfoResponse',
  '2': [
    {'1': 'code', '3': 1, '4': 1, '5': 9, '9': 0, '10': 'code', '17': true},
    {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
    {'1': 'jan', '3': 3, '4': 1, '5': 9, '9': 2, '10': 'jan', '17': true},
    {
      '1': 'table0',
      '3': 4,
      '4': 1,
      '5': 11,
      '6': '.label.v1.GetProdInfoResponse.Table0',
      '9': 3,
      '10': 'table0',
      '17': true
    },
  ],
  '3': [GetProdInfoResponse_Table0$json],
  '8': [
    {'1': '_code'},
    {'1': '_message'},
    {'1': '_jan'},
    {'1': '_table0'},
  ],
};

@$core.Deprecated('Use getProdInfoResponseDescriptor instead')
const GetProdInfoResponse_Table0$json = {
  '1': 'Table0',
  '2': [
    {'1': 'count', '3': 1, '4': 1, '5': 9, '9': 0, '10': 'count', '17': true},
    {
      '1': 'rows',
      '3': 2,
      '4': 3,
      '5': 11,
      '6': '.label.v1.GetProdInfoResponse.Table0.Row',
      '10': 'rows'
    },
  ],
  '3': [GetProdInfoResponse_Table0_Row$json],
  '8': [
    {'1': '_count'},
  ],
};

@$core.Deprecated('Use getProdInfoResponseDescriptor instead')
const GetProdInfoResponse_Table0_Row$json = {
  '1': 'Row',
  '2': [
    {'1': 'row_no', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'rowNo', '17': true},
    {
      '1': 'product_name',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'productName',
      '17': true
    },
    {
      '1': 'sub_name',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'subName',
      '17': true
    },
    {'1': 'brand', '3': 4, '4': 1, '5': 9, '9': 3, '10': 'brand', '17': true},
    {
      '1': 'vendor_name',
      '3': 5,
      '4': 1,
      '5': 9,
      '9': 4,
      '10': 'vendorName',
      '17': true
    },
    {
      '1': 'product_item_cd',
      '3': 6,
      '4': 1,
      '5': 9,
      '9': 5,
      '10': 'productItemCd',
      '17': true
    },
    {
      '1': 'standard_price',
      '3': 7,
      '4': 1,
      '5': 9,
      '9': 6,
      '10': 'standardPrice',
      '17': true
    },
    {
      '1': 'variety_cd',
      '3': 8,
      '4': 1,
      '5': 9,
      '9': 7,
      '10': 'varietyCd',
      '17': true
    },
    {
      '1': 'title_id',
      '3': 9,
      '4': 1,
      '5': 9,
      '9': 8,
      '10': 'titleId',
      '17': true
    },
    {
      '1': 'pattern_id',
      '3': 10,
      '4': 1,
      '5': 9,
      '9': 9,
      '10': 'patternId',
      '17': true
    },
    {
      '1': 'print_num',
      '3': 11,
      '4': 1,
      '5': 9,
      '9': 10,
      '10': 'printNum',
      '17': true
    },
    {
      '1': 'min_print_num',
      '3': 12,
      '4': 1,
      '5': 9,
      '9': 11,
      '10': 'minPrintNum',
      '17': true
    },
    {
      '1': 'max_print_num',
      '3': 13,
      '4': 1,
      '5': 9,
      '9': 12,
      '10': 'maxPrintNum',
      '17': true
    },
  ],
  '8': [
    {'1': '_row_no'},
    {'1': '_product_name'},
    {'1': '_sub_name'},
    {'1': '_brand'},
    {'1': '_vendor_name'},
    {'1': '_product_item_cd'},
    {'1': '_standard_price'},
    {'1': '_variety_cd'},
    {'1': '_title_id'},
    {'1': '_pattern_id'},
    {'1': '_print_num'},
    {'1': '_min_print_num'},
    {'1': '_max_print_num'},
  ],
};

/// Descriptor for `GetProdInfoResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getProdInfoResponseDescriptor = $convert.base64Decode(
    '');
@$core.Deprecated('Use getShelfVarietyCdRequestDescriptor instead')
const GetShelfVarietyCdRequest$json = {
  '1': 'GetShelfVarietyCdRequest',
  '2': [
    {
      '1': 'product_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'productCode',
      '17': true
    },
    {
      '1': 'store_code',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'storeCode',
      '17': true
    },
  ],
  '8': [
    {'1': '_product_code'},
    {'1': '_store_code'},
  ],
};

/// Descriptor for `GetShelfVarietyCdRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getShelfVarietyCdRequestDescriptor =
    $convert.base64Decode(
        'ChhHZXRTaGVsZlZhcmlldHlDZFJlcXVlc3QSJgoMcHJvZHVjdF9jb2RlGAEgASgJSABSC3Byb2R1Y3RDb2RliAEBEiIKCnN0b3JlX2NvZGUYAiABKAlIAVIJc3RvcmVDb2RliAEBQg8KDV9wcm9kdWN0X2NvZGVCDQoLX3N0b3JlX2NvZGU=');
@$core.Deprecated('Use getShelfVarietyCdResponseDescriptor instead')
const GetShelfVarietyCdResponse$json = {
  '1': 'GetShelfVarietyCdResponse',
  '2': [
    {'1': 'code', '3': 1, '4': 1, '5': 9, '9': 0, '10': 'code', '17': true},
    {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
    {
      '1': 'table0',
      '3': 3,
      '4': 1,
      '5': 11,
      '6': '.label.v1.GetShelfVarietyCdResponse.Table0',
      '9': 2,
      '10': 'table0',
      '17': true
    },
  ],
  '3': [GetShelfVarietyCdResponse_Table0$json],
  '8': [
    {'1': '_code'},
    {'1': '_message'},
    {'1': '_table0'},
  ],
};

@$core.Deprecated('Use getShelfVarietyCdResponseDescriptor instead')
const GetShelfVarietyCdResponse_Table0$json = {
  '1': 'Table0',
  '2': [
    {'1': 'count', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'count', '17': true},
    {
      '1': 'rows',
      '3': 2,
      '4': 3,
      '5': 11,
      '6': '.label.v1.GetShelfVarietyCdResponse.Table0.Row',
      '10': 'rows'
    },
  ],
  '3': [GetShelfVarietyCdResponse_Table0_Row$json],
  '8': [
    {'1': '_count'},
  ],
};

@$core.Deprecated('Use getShelfVarietyCdResponseDescriptor instead')
const GetShelfVarietyCdResponse_Table0_Row$json = {
  '1': 'Row',
  '2': [
    {'1': 'row_no', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'rowNo', '17': true},
    {
      '1': 'variety_cd',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'varietyCd',
      '17': true
    },
    {
      '1': 'title_id',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'titleId',
      '17': true
    },
    {
      '1': 'pattern_id',
      '3': 4,
      '4': 1,
      '5': 9,
      '9': 3,
      '10': 'patternId',
      '17': true
    },
  ],
  '8': [
    {'1': '_row_no'},
    {'1': '_variety_cd'},
    {'1': '_title_id'},
    {'1': '_pattern_id'},
  ],
};

/// Descriptor for `GetShelfVarietyCdResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getShelfVarietyCdResponseDescriptor =
    $convert.base64Decode(
        '');
@$core.Deprecated('Use insertPrintHistoryRequestDescriptor instead')
const InsertPrintHistoryRequest$json = {
  '1': 'InsertPrintHistoryRequest',
  '2': [
    {
      '1': 'store_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'storeCode',
      '17': true
    },
    {
      '1': 'print_data_list',
      '3': 2,
      '4': 3,
      '5': 11,
      '6': '.label.v1.InsertPrintHistoryRequest.PrintData',
      '10': 'printDataList'
    },
  ],
  '3': [InsertPrintHistoryRequest_PrintData$json],
  '8': [
    {'1': '_store_code'},
  ],
};

@$core.Deprecated('Use insertPrintHistoryRequestDescriptor instead')
const InsertPrintHistoryRequest_PrintData$json = {
  '1': 'PrintData',
  '2': [
    {
      '1': 'product_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'productCode',
      '17': true
    },
    {'1': 'num', '3': 2, '4': 1, '5': 13, '9': 1, '10': 'num', '17': true},
    {
      '1': 'size_id',
      '3': 3,
      '4': 1,
      '5': 13,
      '9': 2,
      '10': 'sizeId',
      '17': true
    },
    {
      '1': 'title_id',
      '3': 4,
      '4': 1,
      '5': 13,
      '9': 3,
      '10': 'titleId',
      '17': true
    },
    {
      '1': 'pattern_id',
      '3': 5,
      '4': 1,
      '5': 13,
      '9': 4,
      '10': 'patternId',
      '17': true
    },
  ],
  '8': [
    {'1': '_product_code'},
    {'1': '_num'},
    {'1': '_size_id'},
    {'1': '_title_id'},
    {'1': '_pattern_id'},
  ],
};

/// Descriptor for `InsertPrintHistoryRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List insertPrintHistoryRequestDescriptor =
    $convert.base64Decode(
        '');
@$core.Deprecated('Use insertPrintHistoryResponseDescriptor instead')
const InsertPrintHistoryResponse$json = {
  '1': 'InsertPrintHistoryResponse',
  '2': [
    {'1': 'code', '3': 1, '4': 1, '5': 9, '9': 0, '10': 'code', '17': true},
    {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
  ],
  '8': [
    {'1': '_code'},
    {'1': '_message'},
  ],
};

/// Descriptor for `InsertPrintHistoryResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List insertPrintHistoryResponseDescriptor =
    $convert.base64Decode(
        'ChpJbnNlcnRQcmludEhpc3RvcnlSZXNwb25zZRIXCgRjb2RlGAEgASgJSABSBGNvZGWIAQESHQoHbWVzc2FnZRgCIAEoCUgBUgdtZXNzYWdliAEBQgcKBV9jb2RlQgoKCF9tZXNzYWdl');
