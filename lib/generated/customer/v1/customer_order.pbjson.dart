///
//  Generated code. Do not modify.
//  source: customer/v1/customer_order.proto
//
// @dart = 2.12
// ignore_for_file: annotate_overrides,camel_case_types,constant_identifier_names,deprecated_member_use_from_same_package,directives_ordering,library_prefixes,non_constant_identifier_names,prefer_final_fields,return_of_invalid_type,unnecessary_const,unnecessary_import,unnecessary_this,unused_import,unused_shown_name

import 'dart:core' as $core;
import 'dart:convert' as $convert;
import 'dart:typed_data' as $typed_data;

@$core.Deprecated('Use upCustomerQualityMsgRequestDescriptor instead')
const UpCustomerQualityMsgRequest$json = const {
  '1': 'UpCustomerQualityMsgRequest',
  '2': const [
    const {
      '1': 'store_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'storeCode',
      '17': true
    },
    const {
      '1': 'product_code',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'productCode',
      '17': true
    },
    const {
      '1': 'record_number',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'recordNumber',
      '17': true
    },
    const {
      '1': 'order_num',
      '3': 4,
      '4': 1,
      '5': 5,
      '9': 3,
      '10': 'orderNum',
      '17': true
    },
    const {
      '1': 'receipt_date',
      '3': 5,
      '4': 1,
      '5': 11,
      '6': '.customer.v1.UpCustomerQualityMsgRequest.Date',
      '9': 4,
      '10': 'receiptDate',
      '17': true
    },
  ],
  '3': const [UpCustomerQualityMsgRequest_Date$json],
  '8': const [
    const {'1': '_store_code'},
    const {'1': '_product_code'},
    const {'1': '_record_number'},
    const {'1': '_order_num'},
    const {'1': '_receipt_date'},
  ],
};

@$core.Deprecated('Use upCustomerQualityMsgRequestDescriptor instead')
const UpCustomerQualityMsgRequest_Date$json = const {
  '1': 'Date',
  '2': const [
    const {'1': 'year', '3': 1, '4': 1, '5': 13, '10': 'year'},
    const {'1': 'month', '3': 2, '4': 1, '5': 13, '10': 'month'},
    const {'1': 'day', '3': 3, '4': 1, '5': 13, '10': 'day'},
  ],
};

/// Descriptor for `UpCustomerQualityMsgRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List upCustomerQualityMsgRequestDescriptor =
    $convert.base64Decode(
        '');
@$core.Deprecated('Use upCustomerQualityMsgResponseDescriptor instead')
const UpCustomerQualityMsgResponse$json = const {
  '1': 'UpCustomerQualityMsgResponse',
  '2': const [
    const {
      '1': 'code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'code',
      '17': true
    },
    const {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_code'},
    const {'1': '_message'},
  ],
};

/// Descriptor for `UpCustomerQualityMsgResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List upCustomerQualityMsgResponseDescriptor =
    $convert.base64Decode(
        'ChxVcEN1c3RvbWVyUXVhbGl0eU1zZ1Jlc3BvbnNlEhcKBGNvZGUYASABKAlIAFIEY29kZYgBARIdCgdtZXNzYWdlGAIgASgJSAFSB21lc3NhZ2WIAQFCBwoFX2NvZGVCCgoIX21lc3NhZ2U=');
@$core.Deprecated('Use getProdInfoNewRequestDescriptor instead')
const GetProdInfoNewRequest$json = const {
  '1': 'GetProdInfoNewRequest',
  '2': const [
    const {
      '1': 'product_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'productCode',
      '17': true
    },
    const {
      '1': 'store_code',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'storeCode',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_product_code'},
    const {'1': '_store_code'},
  ],
};

/// Descriptor for `GetProdInfoNewRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getProdInfoNewRequestDescriptor = $convert.base64Decode(
    'ChVHZXRQcm9kSW5mb05ld1JlcXVlc3QSJgoMcHJvZHVjdF9jb2RlGAEgASgJSABSC3Byb2R1Y3RDb2RliAEBEiIKCnN0b3JlX2NvZGUYAiABKAlIAVIJc3RvcmVDb2RliAEBQg8KDV9wcm9kdWN0X2NvZGVCDQoLX3N0b3JlX2NvZGU=');
@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse$json = const {
  '1': 'GetProdInfoNewResponse',
  '2': const [
    const {
      '1': 'code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'code',
      '17': true
    },
    const {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
    const {
      '1': 'table0',
      '3': 3,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table0',
      '10': 'table0'
    },
    const {
      '1': 'table1',
      '3': 4,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table1',
      '10': 'table1'
    },
    const {
      '1': 'table2',
      '3': 5,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table2',
      '10': 'table2'
    },
    const {
      '1': 'table3',
      '3': 6,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table3',
      '10': 'table3'
    },
    const {
      '1': 'table4',
      '3': 7,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table4',
      '10': 'table4'
    },
    const {
      '1': 'table5',
      '3': 8,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table5',
      '10': 'table5'
    },
    const {
      '1': 'table6',
      '3': 9,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table6',
      '10': 'table6'
    },
    const {
      '1': 'table7',
      '3': 10,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table7',
      '10': 'table7'
    },
    const {
      '1': 'table8',
      '3': 11,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetProdInfoNewResponse.Table8',
      '10': 'table8'
    },
  ],
  '3': const [
    GetProdInfoNewResponse_Table0$json,
    GetProdInfoNewResponse_Table1$json,
    GetProdInfoNewResponse_Table2$json,
    GetProdInfoNewResponse_Table3$json,
    GetProdInfoNewResponse_Table4$json,
    GetProdInfoNewResponse_Table5$json,
    GetProdInfoNewResponse_Table6$json,
    GetProdInfoNewResponse_Table7$json,
    GetProdInfoNewResponse_Table8$json
  ],
  '8': const [
    const {'1': '_code'},
    const {'1': '_message'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table0$json = const {
  '1': 'Table0',
  '2': const [
    const {
      '1': 'prd_nm',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'prdNm',
      '17': true
    },
    const {
      '1': 'info1',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'info1',
      '17': true
    },
    const {
      '1': 'info2',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'info2',
      '17': true
    },
    const {
      '1': 'unit_cp',
      '3': 4,
      '4': 1,
      '5': 1,
      '9': 3,
      '10': 'unitCp',
      '17': true
    },
    const {
      '1': 'unit_rp',
      '3': 5,
      '4': 1,
      '5': 5,
      '9': 4,
      '10': 'unitRp',
      '17': true
    },
    const {
      '1': 'pack_num',
      '3': 6,
      '4': 1,
      '5': 5,
      '9': 5,
      '10': 'packNum',
      '17': true
    },
    const {
      '1': 'odr_unit',
      '3': 7,
      '4': 1,
      '5': 5,
      '9': 6,
      '10': 'odrUnit',
      '17': true
    },
    const {
      '1': 'rnb_stk',
      '3': 8,
      '4': 1,
      '5': 9,
      '9': 7,
      '10': 'rnbStk',
      '17': true
    },
    const {
      '1': 'order_method',
      '3': 9,
      '4': 1,
      '5': 9,
      '9': 8,
      '10': 'orderMethod',
      '17': true
    },
    const {
      '1': 'rmd_num',
      '3': 10,
      '4': 1,
      '5': 9,
      '9': 9,
      '10': 'rmdNum',
      '17': true
    },
    const {
      '1': 'stk_num',
      '3': 11,
      '4': 1,
      '5': 9,
      '9': 10,
      '10': 'stkNum',
      '17': true
    },
    const {
      '1': 'vdr_nm',
      '3': 12,
      '4': 1,
      '5': 9,
      '9': 11,
      '10': 'vdrNm',
      '17': true
    },
    const {
      '1': 'sct_nm',
      '3': 13,
      '4': 1,
      '5': 9,
      '9': 12,
      '10': 'sctNm',
      '17': true
    },
    const {
      '1': 'odr_stp',
      '3': 14,
      '4': 1,
      '5': 5,
      '9': 13,
      '10': 'odrStp',
      '17': true
    },
    const {
      '1': 'prd_color',
      '3': 15,
      '4': 1,
      '5': 9,
      '9': 14,
      '10': 'prdColor',
      '17': true
    },
    const {
      '1': 'prd_size',
      '3': 16,
      '4': 1,
      '5': 9,
      '9': 15,
      '10': 'prdSize',
      '17': true
    },
    const {
      '1': 'is_100_yen',
      '3': 17,
      '4': 1,
      '5': 5,
      '9': 16,
      '10': 'is100Yen',
      '17': true
    },
    const {
      '1': 'level2',
      '3': 18,
      '4': 1,
      '5': 5,
      '9': 17,
      '10': 'level2',
      '17': true
    },
    const {
      '1': 'level3',
      '3': 19,
      '4': 1,
      '5': 5,
      '9': 18,
      '10': 'level3',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_prd_nm'},
    const {'1': '_info1'},
    const {'1': '_info2'},
    const {'1': '_unit_cp'},
    const {'1': '_unit_rp'},
    const {'1': '_pack_num'},
    const {'1': '_odr_unit'},
    const {'1': '_rnb_stk'},
    const {'1': '_order_method'},
    const {'1': '_rmd_num'},
    const {'1': '_stk_num'},
    const {'1': '_vdr_nm'},
    const {'1': '_sct_nm'},
    const {'1': '_odr_stp'},
    const {'1': '_prd_color'},
    const {'1': '_prd_size'},
    const {'1': '_is_100_yen'},
    const {'1': '_level2'},
    const {'1': '_level3'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table1$json = const {
  '1': 'Table1',
  '2': const [
    const {
      '1': 'tana_no',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'tanaNo',
      '17': true
    },
    const {
      '1': 'step_no',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'stepNo',
      '17': true
    },
    const {
      '1': 'row_no',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'rowNo',
      '17': true
    },
    const {
      '1': 'prd_cd',
      '3': 4,
      '4': 1,
      '5': 9,
      '9': 3,
      '10': 'prdCd',
      '17': true
    },
    const {
      '1': 'odr_num',
      '3': 5,
      '4': 1,
      '5': 5,
      '9': 4,
      '10': 'odrNum',
      '17': true
    },
    const {
      '1': 'rmd_odr',
      '3': 6,
      '4': 1,
      '5': 9,
      '9': 5,
      '10': 'rmdOdr',
      '17': true
    },
    const {
      '1': 'sales_sum',
      '3': 7,
      '4': 1,
      '5': 9,
      '9': 6,
      '10': 'salesSum',
      '17': true
    },
    const {
      '1': 'user_cd',
      '3': 8,
      '4': 1,
      '5': 9,
      '9': 7,
      '10': 'userCd',
      '17': true
    },
    const {
      '1': 'scan_flg',
      '3': 9,
      '4': 1,
      '5': 5,
      '9': 8,
      '10': 'scanFlg',
      '17': true
    },
    const {
      '1': 'imp_flg',
      '3': 10,
      '4': 1,
      '5': 5,
      '9': 9,
      '10': 'impFlg',
      '17': true
    },
    const {
      '1': 'conf_flg',
      '3': 11,
      '4': 1,
      '5': 5,
      '9': 10,
      '10': 'confFlg',
      '17': true
    },
    const {
      '1': 'odr_num_p0',
      '3': 12,
      '4': 1,
      '5': 9,
      '9': 11,
      '10': 'odrNumP0',
      '17': true
    },
    const {
      '1': 'odr_num_p1',
      '3': 13,
      '4': 1,
      '5': 9,
      '9': 12,
      '10': 'odrNumP1',
      '17': true
    },
    const {
      '1': 'odr_num_p2',
      '3': 14,
      '4': 1,
      '5': 9,
      '9': 13,
      '10': 'odrNumP2',
      '17': true
    },
    const {
      '1': 'odr_num_p3',
      '3': 15,
      '4': 1,
      '5': 9,
      '9': 14,
      '10': 'odrNumP3',
      '17': true
    },
    const {
      '1': 'odr_num_p4',
      '3': 16,
      '4': 1,
      '5': 9,
      '9': 15,
      '10': 'odrNumP4',
      '17': true
    },
    const {
      '1': 'odr_num_p5',
      '3': 17,
      '4': 1,
      '5': 9,
      '9': 16,
      '10': 'odrNumP5',
      '17': true
    },
    const {
      '1': 'odr_num_p6',
      '3': 18,
      '4': 1,
      '5': 9,
      '9': 17,
      '10': 'odrNumP6',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_tana_no'},
    const {'1': '_step_no'},
    const {'1': '_row_no'},
    const {'1': '_prd_cd'},
    const {'1': '_odr_num'},
    const {'1': '_rmd_odr'},
    const {'1': '_sales_sum'},
    const {'1': '_user_cd'},
    const {'1': '_scan_flg'},
    const {'1': '_imp_flg'},
    const {'1': '_conf_flg'},
    const {'1': '_odr_num_p0'},
    const {'1': '_odr_num_p1'},
    const {'1': '_odr_num_p2'},
    const {'1': '_odr_num_p3'},
    const {'1': '_odr_num_p4'},
    const {'1': '_odr_num_p5'},
    const {'1': '_odr_num_p6'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table2$json = const {
  '1': 'Table2',
  '2': const [
    const {
      '1': 'reason_name',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'reasonName',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_reason_name'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table3$json = const {
  '1': 'Table3',
  '2': const [
    const {
      '1': 'record_number',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'recordNumber',
      '17': true
    },
    const {
      '1': 'order_date',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'orderDate',
      '17': true
    },
    const {
      '1': 'receive_date',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'receiveDate',
      '17': true
    },
    const {
      '1': 'employee_name',
      '3': 4,
      '4': 1,
      '5': 9,
      '9': 3,
      '10': 'employeeName',
      '17': true
    },
    const {
      '1': 'order_quantify',
      '3': 5,
      '4': 1,
      '5': 5,
      '9': 4,
      '10': 'orderQuantify',
      '17': true
    },
    const {
      '1': 'flag',
      '3': 6,
      '4': 1,
      '5': 9,
      '9': 5,
      '10': 'flag',
      '17': true
    },
    const {
      '1': 'jan_status',
      '3': 7,
      '4': 1,
      '5': 9,
      '9': 6,
      '10': 'janStatus',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_record_number'},
    const {'1': '_order_date'},
    const {'1': '_receive_date'},
    const {'1': '_employee_name'},
    const {'1': '_order_quantify'},
    const {'1': '_flag'},
    const {'1': '_jan_status'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table4$json = const {
  '1': 'Table4',
  '2': const [
    const {'1': 'flg', '3': 1, '4': 1, '5': 9, '9': 0, '10': 'flg', '17': true},
    const {
      '1': 'branch_whq',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'branchWhq',
      '17': true
    },
    const {
      '1': 'color_flg',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'colorFlg',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_flg'},
    const {'1': '_branch_whq'},
    const {'1': '_color_flg'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table5$json = const {
  '1': 'Table5',
  '2': const [
    const {
      '1': 'id_num',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'idNum',
      '17': true
    },
    const {'1': 'div', '3': 2, '4': 1, '5': 9, '9': 1, '10': 'div', '17': true},
    const {
      '1': 'end_cd',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'endCd',
      '17': true
    },
    const {
      '1': 'end_nm',
      '3': 4,
      '4': 1,
      '5': 9,
      '9': 3,
      '10': 'endNm',
      '17': true
    },
    const {
      '1': 'display_cnt',
      '3': 5,
      '4': 1,
      '5': 9,
      '9': 4,
      '10': 'displayCnt',
      '17': true
    },
    const {
      '1': 'display_cnt1',
      '3': 6,
      '4': 1,
      '5': 9,
      '9': 5,
      '10': 'displayCnt1',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_id_num'},
    const {'1': '_div'},
    const {'1': '_end_cd'},
    const {'1': '_end_nm'},
    const {'1': '_display_cnt'},
    const {'1': '_display_cnt1'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table6$json = const {
  '1': 'Table6',
  '2': const [
    const {'1': 'flg', '3': 1, '4': 1, '5': 5, '9': 0, '10': 'flg', '17': true},
    const {
      '1': 'pda_cnt',
      '3': 2,
      '4': 1,
      '5': 5,
      '9': 1,
      '10': 'pdaCnt',
      '17': true
    },
    const {
      '1': 'prod_nm',
      '3': 3,
      '4': 1,
      '5': 9,
      '9': 2,
      '10': 'prodNm',
      '17': true
    },
    const {
      '1': 'standard_nm',
      '3': 4,
      '4': 1,
      '5': 9,
      '9': 3,
      '10': 'standardNm',
      '17': true
    },
    const {
      '1': 'prd_color',
      '3': 5,
      '4': 1,
      '5': 9,
      '9': 4,
      '10': 'prdColor',
      '17': true
    },
    const {
      '1': 'prd_size',
      '3': 6,
      '4': 1,
      '5': 9,
      '9': 5,
      '10': 'prdSize',
      '17': true
    },
    const {
      '1': 'order_method',
      '3': 7,
      '4': 1,
      '5': 5,
      '9': 6,
      '10': 'orderMethod',
      '17': true
    },
    const {
      '1': 'order_flg',
      '3': 8,
      '4': 1,
      '5': 5,
      '9': 7,
      '10': 'orderFlg',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_flg'},
    const {'1': '_pda_cnt'},
    const {'1': '_prod_nm'},
    const {'1': '_standard_nm'},
    const {'1': '_prd_color'},
    const {'1': '_prd_size'},
    const {'1': '_order_method'},
    const {'1': '_order_flg'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table7$json = const {
  '1': 'Table7',
  '2': const [
    const {
      '1': 'order_date',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'orderDate',
      '17': true
    },
    const {
      '1': 'receipt_date',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'receiptDate',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_order_date'},
    const {'1': '_receipt_date'},
  ],
};

@$core.Deprecated('Use getProdInfoNewResponseDescriptor instead')
const GetProdInfoNewResponse_Table8$json = const {
  '1': 'Table8',
  '2': const [
    const {
      '1': 'current_time',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'currentTime',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_current_time'},
  ],
};

/// Descriptor for `GetProdInfoNewResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getProdInfoNewResponseDescriptor =
    $convert.base64Decode(
        '');
@$core.Deprecated('Use getReceveDateForCheckRequestDescriptor instead')
const GetReceveDateForCheckRequest$json = const {
  '1': 'GetReceveDateForCheckRequest',
  '2': const [
    const {
      '1': 'store_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'storeCode',
      '17': true
    },
    const {
      '1': 'product_code',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'productCode',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_store_code'},
    const {'1': '_product_code'},
  ],
};

/// Descriptor for `GetReceveDateForCheckRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getReceveDateForCheckRequestDescriptor =
    $convert.base64Decode(
        'ChxHZXRSZWNldmVEYXRlRm9yQ2hlY2tSZXF1ZXN0EiIKCnN0b3JlX2NvZGUYASABKAlIAFIJc3RvcmVDb2RliAEBEiYKDHByb2R1Y3RfY29kZRgCIAEoCUgBUgtwcm9kdWN0Q29kZYgBAUINCgtfc3RvcmVfY29kZUIPCg1fcHJvZHVjdF9jb2Rl');
@$core.Deprecated('Use getReceveDateForCheckResponseDescriptor instead')
const GetReceveDateForCheckResponse$json = const {
  '1': 'GetReceveDateForCheckResponse',
  '2': const [
    const {
      '1': 'code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'code',
      '17': true
    },
    const {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
    const {
      '1': 'table0',
      '3': 3,
      '4': 3,
      '5': 11,
      '6': '.customer.v1.GetReceveDateForCheckResponse.Table0',
      '10': 'table0'
    },
  ],
  '3': const [GetReceveDateForCheckResponse_Table0$json],
  '8': const [
    const {'1': '_code'},
    const {'1': '_message'},
  ],
};

@$core.Deprecated('Use getReceveDateForCheckResponseDescriptor instead')
const GetReceveDateForCheckResponse_Table0$json = const {
  '1': 'Table0',
  '2': const [
    const {
      '1': 'order_date',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'orderDate',
      '17': true
    },
    const {
      '1': 'receipt_date',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'receiptDate',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_order_date'},
    const {'1': '_receipt_date'},
  ],
};

/// Descriptor for `GetReceveDateForCheckResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List getReceveDateForCheckResponseDescriptor =
    $convert.base64Decode(
        '');
@$core.Deprecated('Use setOrderNumNewRequestDescriptor instead')
const SetOrderNumNewRequest$json = const {
  '1': 'SetOrderNumNewRequest',
  '2': const [
    const {
      '1': 'product_code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'productCode',
      '17': true
    },
    const {
      '1': 'store_code',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'storeCode',
      '17': true
    },
    const {
      '1': 'order_num',
      '3': 3,
      '4': 1,
      '5': 5,
      '9': 2,
      '10': 'orderNum',
      '17': true
    },
    const {
      '1': 'order_date',
      '3': 4,
      '4': 1,
      '5': 11,
      '6': '.customer.v1.SetOrderNumNewRequest.Date',
      '9': 3,
      '10': 'orderDate',
      '17': true
    },
    const {
      '1': 'receipt_date',
      '3': 5,
      '4': 1,
      '5': 11,
      '6': '.customer.v1.SetOrderNumNewRequest.Date',
      '9': 4,
      '10': 'receiptDate',
      '17': true
    },
    const {
      '1': 'order_brand',
      '3': 6,
      '4': 1,
      '5': 9,
      '9': 5,
      '10': 'orderBrand',
      '17': true
    },
    const {
      '1': 'order_sale_price',
      '3': 7,
      '4': 1,
      '5': 5,
      '9': 6,
      '10': 'orderSalePrice',
      '17': true
    },
    const {
      '1': 'order_spec',
      '3': 8,
      '4': 1,
      '5': 9,
      '9': 7,
      '10': 'orderSpec',
      '17': true
    },
    const {
      '1': 'order_units',
      '3': 9,
      '4': 1,
      '5': 3,
      '9': 8,
      '10': 'orderUnits',
      '17': true
    },
  ],
  '3': const [SetOrderNumNewRequest_Date$json],
  '8': const [
    const {'1': '_product_code'},
    const {'1': '_store_code'},
    const {'1': '_order_num'},
    const {'1': '_order_date'},
    const {'1': '_receipt_date'},
    const {'1': '_order_brand'},
    const {'1': '_order_sale_price'},
    const {'1': '_order_spec'},
    const {'1': '_order_units'},
  ],
};

@$core.Deprecated('Use setOrderNumNewRequestDescriptor instead')
const SetOrderNumNewRequest_Date$json = const {
  '1': 'Date',
  '2': const [
    const {'1': 'year', '3': 1, '4': 1, '5': 13, '10': 'year'},
    const {'1': 'month', '3': 2, '4': 1, '5': 13, '10': 'month'},
    const {'1': 'day', '3': 3, '4': 1, '5': 13, '10': 'day'},
  ],
};

/// Descriptor for `SetOrderNumNewRequest`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List setOrderNumNewRequestDescriptor = $convert.base64Decode(
    '');
@$core.Deprecated('Use setOrderNumNewResponseDescriptor instead')
const SetOrderNumNewResponse$json = const {
  '1': 'SetOrderNumNewResponse',
  '2': const [
    const {
      '1': 'code',
      '3': 1,
      '4': 1,
      '5': 9,
      '9': 0,
      '10': 'code',
      '17': true
    },
    const {
      '1': 'message',
      '3': 2,
      '4': 1,
      '5': 9,
      '9': 1,
      '10': 'message',
      '17': true
    },
  ],
  '8': const [
    const {'1': '_code'},
    const {'1': '_message'},
  ],
};

/// Descriptor for `SetOrderNumNewResponse`. Decode as a `google.protobuf.DescriptorProto`.
final $typed_data.Uint8List setOrderNumNewResponseDescriptor =
    $convert.base64Decode(
        'ChZTZXRPcmRlck51bU5ld1Jlc3BvbnNlEhcKBGNvZGUYASABKAlIAFIEY29kZYgBARIdCgdtZXNzYWdlGAIgASgJSAFSB21lc3NhZ2WIAQFCBwoFX2NvZGVCCgoIX21lc3NhZ2U=');
